...
#Main Menu Function
def main_menu():
    title_font = pygame.font.SysFont("Constantia", 74)
    prompt_font = pygame.font.SysFont("Constantia", 36)
    while True:
...
def pause_menu():
    ...
    while True:
        ...
        resume_text = prompt_font.render("Press 'P' to Resume", True, WHITE)
        ...
        clock.tick(FPS)
# Health Bar Function
def draw_health_bar(surface, x, y, health, max_health, bar_width, bar_height):
    health_ratio = max(0, health / max_health)  # Safeguard against negative health
    pygame.draw.rect(surface, RED, (x, y, bar_width, bar_height))  # Background (lost health)
    pygame.draw.rect(surface, GREEN, (x, y, bar_width * health_ratio, bar_height))  # Foreground (remaining health)
    pygame.draw.rect(surface, WHITE, (x, y, bar_width, bar_height), 2)  # Optional border
...
def main():
    ...
    while running:
        ...
        draw_health_bar(screen, 10, 10, player.health, MAX_HEALTH, 200, 20)
        ...
        draw_text(f"Health: {player.health}", BLACK, 10, 10)
